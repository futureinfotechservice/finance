    ...
    try {
        ...
        foreach ($paymentdata as $payment) {
            ...
            if (isset($payment['selected']) && $payment['selected'] == true) {
                ...
                // Check if payment is overdue based on COLLECTION DATE
                $isOverdue = false;
                ...
                if (!$isOverdue && $penalty_received > 0) {
                    throw new Exception("Cannot collect penalty on EMI $dueno. It is not overdue (collection date: $collectiondate, due date: {$paymentDetails['duedate']}).");
                }
                ...
                $newDueno = null;
                $newDueDate = null;
                $olddueno = null; // Store the original dueno
                ...
                if ($shouldCreateNewEMI) {
                    ...
                    $olddueno = $dueno; // Store the original dueno
                    
                    // Insert new schedule entry for unpaid amount with next weekday date
                    // Save olddueno in the new record
                    $insertScheduleSql = "INSERT INTO loanschedule 
                                         (loanid, companyid, dueno, duedate, dueamount, status, olddueno) 
                                         VALUES ('$loanid', '$companyid', '$newDueno', 
                                                 '$newDueDate', '$dueamount', 'Pending', '$olddueno')";
                    
                    if (!mysqli_query($conn, $insertScheduleSql)) {
                        throw new Exception("Failed to create new schedule: " . mysqli_error($conn));
                    }
                    
                    // Also update the original schedule record to mark it as having created a new EMI
                    // $updateOldSql = "UPDATE loanschedule 
                                    // SET olddueno = '$dueno' 
                                    // WHERE loanid = '$loanid' 
                                    // AND companyid = '$companyid' 
                                    // AND dueno = '$dueno'";
                    
                    // if (!mysqli_query($conn, $updateOldSql)) {
                        // throw new Exception("Failed to update old schedule record: " . mysqli_error($conn));
                    // }
                ...
                $collectedDuenos[] = [
                    ...
                    'olddueno' => $olddueno, // Original dueno that was moved
                    'payment_type' => 'unpaid',
