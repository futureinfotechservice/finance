...
try {
    ...
    if (empty($companyid) || empty($serial_no) || empty($date) || empty($payment_account) || empty($payment_account_id)) {
    ...
    $checkSql = "SELECT * FROM payment_entry WHERE serial_no = '$serial_no' AND companyid = '$companyid'";
    ...
    $sql = "INSERT INTO payment_entry 
            (companyid, serial_no, date, payment_account, payment_account_id, cash_bank, amount, description, addedby) 
            VALUES ('$companyid', '$serial_no', '$date', '$payment_account', '$payment_account_id', '$cash_bank', '$amount', '$description', '$addedby')";

    if (mysqli_query($conn, $sql)) {
        $response["status"] = "success";
        $response["message"] = "Payment entry created successfully";
