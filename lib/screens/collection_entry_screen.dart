    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _totalBalanceController.text = _totals['loanBalance']?.toString() ?? '0.00';
...
// Add this helper function to your _CollectionEntryScreenState class
  ...
  void _togglePaymentSelection(int index, String type) {
    ...
    if (type == 'unpaid') {
      ...
      if (!isOverdue) {
        // Show error message if trying to select unpaid for non-overdue payment
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
    ...
    setState(() {
      ...
      // DO NOT update loan balance and penalty balance fields here
      // Only update the totals display at the bottom
      _updateTotalsOnly();
    });
  }
  ...
  void _updateTotalsOnly() {
    ...
    for (int i = 0; i < _selectedPayments.length; i++) {
      if (_selectedPayments[i]['selected'] == true) {
        ...
        // Penalty is 0 for success payments
      } else if (_selectedPayments[i]['unpaid'] == true) {
        // Payment unpaid - include penalty only (NO due amount)
        totalPenalty += double.parse(_selectedPayments[i]['penaltyamount'].toString());
        // No due amount collected for unpaid payments
      }
    }

    // Update only the totals display, NOT the balance fields
    setState(() {
      _totalSelectedAmount = totalDue;
      _totalSelectedPenalty = totalPenalty;
    });

    print("📊 Updated selection totals - Payment: ₹$totalDue, Penalty: ₹$totalPenalty");
    print("⚠️ Loan Balance and Penalty Balance fields remain unchanged until collection is recorded");
  }
  ...
  void _calculateTotals() {
    ...
    // Calculate what the new balances would be, but DO NOT update the fields
    double currentLoanBalance = double.tryParse(_loanBalanceController.text) ?? 0.0;
    double currentPenaltyBalance = double.tryParse(_penaltyBalanceController.text) ?? 0.0;

    double newLoanBalance = currentLoanBalance - totalDue;
    double newPenaltyBalance = currentPenaltyBalance - totalPenalty;
    double newTotalBalance = newLoanBalance + newPenaltyBalance;

    // Only update the display totals, NOT the balance fields
    setState(() {
      _totalSelectedAmount = totalDue;
      _totalSelectedPenalty = totalPenalty;
    });

    print("📊 Calculated totals - Payment: ₹$totalDue, Penalty: ₹$totalPenalty");
    print("💡 Current Loan Balance: ₹$currentLoanBalance");
    print("💡 Current Penalty Balance: ₹$currentPenaltyBalance");
    print("💡 Would update to - Loan Balance: ₹$newLoanBalance, Penalty Balance: ₹$newPenaltyBalance");
    print("⚠️ Balance fields remain unchanged until collection is recorded");
  }
  ...
  Future<void> _processCollection() async {
    ...
    try {
      ...
      for (int i = 0; i < _selectedPayments.length; i++) {
        ...
        if (payment['selected'] == true) {
          ...
          paymentData.add({
            ...
            'paidamount': payment['dueamount'].toString(), // Full amount paid
            'penaltyamount': '0.00', // No penalty for overdue payments when paid
            'selected': true,
            'unpaid': false,
          });
          ...
          paymentData.add({
            'dueno': payment['dueno'],
            'dueamount': payment['dueamount'].toString(),
            'paidamount': '0.00', // No amount paid for unpaid
            'penaltyamount': penaltyToSave.toStringAsFixed(2), // Fixed penalty or 0
            'selected': false,
            'unpaid': true,
          });
        }
      ...
      if (result == "Success" && mounted) {
        // Calculate new balances AFTER successful collection
        double currentLoanBalance = double.tryParse(_loanBalanceController.text) ?? 0.0;
        double currentPenaltyBalance = double.tryParse(_penaltyBalanceController.text) ?? 0.0;

        // Update balance fields ONLY after successful collection
        setState(() {
          _loanBalanceController.text = (currentLoanBalance - _totalSelectedAmount).toStringAsFixed(2);
          _penaltyBalanceController.text = (currentPenaltyBalance - _totalSelectedPenalty).toStringAsFixed(2);

          // Update total balance
          double newLoanBalance = currentLoanBalance - _totalSelectedAmount;
          double newPenaltyBalance = currentPenaltyBalance - _totalSelectedPenalty;
        ...
        successMessage += 'Paid Amount: ₹${_totalSelectedAmount.toStringAsFixed(2)}\n';
        successMessage += 'Penalty Collected: ₹${_totalSelectedPenalty.toStringAsFixed(2)}\n';
        successMessage += 'Updated Loan Balance: ₹${_loanBalanceController.text}\n';
        successMessage += 'Updated Penalty Balance: ₹${_penaltyBalanceController.text}';
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  const Text(
                                    'Paid: Available for all payments',
                                    style: TextStyle(
                                      fontSize: 12,
                                      color: Colors.green,
                                    ),
                              ...
                              Row(
                                children: [
                                  ...
                                  const SizedBox(width: 8),
                                  const Text(
                                    'Unpaid: Only available for overdue payments',
                                    style: TextStyle(
                                      fontSize: 12,
                                      color: Colors.red,
                                    ),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    const Text(
                      '• Paid (✓): Available for ALL payments - collects full due amount, NO penalty even if overdue',
                      style: TextStyle(
                        fontSize: 12,
                        color: Colors.blue,
                      ),
                    ...
                    const Text(
                      '• Unpaid (✗): ONLY available for OVERDUE payments - collects fixed penalty, due amount moves to next weekday',
                      style: TextStyle(
                        fontSize: 12,
                        color: Colors.blue,
                      ),
                    ...
                    const Text(
                      '• Unpaid (✗): NOT available for payments that are NOT overdue',
                      style: TextStyle(
                        fontSize: 12,
                        color: Colors.blue,
                        fontStyle: FontStyle.italic,
                    ...
                    if (nextWeekday.isNotEmpty)
                      Text(
                        '• Unpaid EMI will be scheduled for: $nextWeekday (Next weekday from last due date)',
                        style: const TextStyle(
                          fontSize: 12,
                          color: Colors.blue,
                          fontWeight: FontWeight.bold,
